...
import { quizContent, storyContent } from "../lib/openAI/prompts/wstmContent.ts";
  ...
  handler: async (
    ...
    if (args.length === 0) {
      ...
      await client.say(
        ...
        `${isStoryMode ? "Who sent this Message : Story" : isQuizMode ? "Who sent this Message : Quiz" : "Who sent this Message"} | 3 Versuche und ${isStoryMode ? "5" : isQuizMode ? "4" : "3"} Minuten | ${isStoryMode ? "+ws <user>" : isQuizMode ? "+ws <user>" : "+ws <user>"} |  COGGERS loading...`
      );
      ...
      if (quizMode) {
        client.on("message", (channel: string, userstate: ChatUserstate, message: string) => {
          ...
          if (
            ...
            client.say(
              channel,
              `${userName}, Deine Antwort: ${answer} ${answer === "✅" ? "Es war richtig!" : "Es war falsch!"
              }`
            );
